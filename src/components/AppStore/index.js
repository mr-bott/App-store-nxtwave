...
import AppItem from '../AppItem/index.js'
...
class AppStore extends Component {
  state = {actabId: tabsList[0].tabId, search: ''}

  clicked = tabId => {
    this.setState({actabId: tabId})
  }
  change = event => {
  ...
  render() {
    const {actabId, search} = this.state
    const selected = appsList.filter(each => actabId === each.category)
    const searched = selected.filter(each =>
